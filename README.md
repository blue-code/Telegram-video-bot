...
- **eBook 라이브러리** - EPUB 파일 자동 인식, 표지/제목/작가 메타데이터 추출, 전용 그리드 뷰 및 검색 제공
    - **시리즈 관리**: 자동 시리즈 그룹핑뿐만 아니라, 사용자가 직접 시리즈를 생성하고 책을 관리할 수 있는 기능 지원.
- **EPUB 리더** - 웹에서 EPUB 전자책 읽기 지원 (진행 상황 자동 저장, 폰트/테마 설정, 터치 인터페이스)
    - **북마크**: 읽던 위치를 저장하고 메모를 남길 수 있는 북마크 기능 지원.
- **TTS 오디오북** - EPUB 텍스트를 고품질 음성(Edge TTS)으로 낭독 듣기 지원 (안정성 대폭 강화 및 끊김 없는 재생 구현)
- **만화/코믹스 뷰어** - ZIP/CBZ 파일 지원, 이미지 최적화, 좌우 터치 네비게이션, 이어보기
    - 지능형 시리즈 인식: `제목 9화`, `제목 - 01화`, `[작가] 제목 01`, `제목 (01)`, `Karamitsuku Shisen 휘감기는 시선 1` 등 다양한 파일명 패턴 자동 분류 지원.
- **최근 목록** - 최근 다운로드한 영상 바로 접근
- **페이지네이션** - 대량의 영상도 편리하게 탐색
- **캐시 관리** - 대시보드에서 다운로드 캐시 및 인코딩 임시 파일을 원클릭으로 정리하여 서버 용량 확보 가능.
